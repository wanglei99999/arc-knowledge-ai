...
**v7.0 设计中**：多租户模型路由，修复 v6.0 全局模型状态局限。**v8.0 设计中**：模型用量追踪与费用配额，依赖 v7.0。
...
| **v7.0** | 多租户模型路由（三层解析 + TenantConfig 持久化 + 删除全局 model_state） | 📋 设计中 |
| **v8.0** | 模型用量追踪（token 计量 + 定价表 + 费用配额，依赖 v7.0） | 📋 设计中 |
