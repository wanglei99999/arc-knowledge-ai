...
class RRFFusionStage(BaseStage[SearchContext, SearchContext]):
    """
    Reciprocal Rank Fusion（RRF）融合 Stage。

    将向量检索和关键词检索的结果列表合并，
    按 RRF 分数重新排序后将结果写入 SearchContext.ranked_hits，
    保持 SearchContext 作为 data carrier 贯穿整条 Retrieval Pipeline。
